...
$server_ip="139.59.38.160";
  $response = array("foods"=>array(),"shop"=>array(),"second"=>array(),"bookings"=>array(),"timings"=>array(),"drivers"=>array(),"vehicle"=>array());
    ...
    $eats=mysqli_query($conn, "SELECT f.ID,f.IDMenu,f.IDSubsubmenu,(SELECT Name FROM subsubmenu WHERE ID=f.IDSubmenu) AS `Subsubmenu`,(SELECT Category FROM submenu WHERE ID=f.IDSubsubmenu) AS `Submenu`,m.Name AS Menu,f.Name,f.Weight,f.Unit,f.Description,f.MRP,f.JalpanPrice,f.Discount,f.Photo FROM `foods` f   INNER JOIN menu_type m ON m.ID=f.IDMenu WHERE f.Available=1  AND f.IDMenu='$food'");
...
array_push($response["foods"], $jsonRow_201);
    ...
    $timings=mysqli_query($conn, "SELECT t.Name,t.Address,b.Type,b.Unique_Ride_Code,b.From_Address,b.pCost,b.ID,b.Cost,b.PaymentMode,b.PaymentVerified,b.Is_Paid,b.ETR,(SELECT Name FROM user_details WHERE ID=b.Driver_ID) AS `Driver`,(SELECT Photo FROM user_details WHERE ID=b.Driver_ID) AS `Photo`,b.Vehicle_ID,b.Booking_Date,b.Booking_Time,d.Delivered,b.End_Date,b.End_Time FROM `store_order` s INNER JOIN delievered d ON s.OrderID=d.OrderID INNER JOIN book_ride b ON b.OTP=d.OrderID INNER JOIN tez_Canteen t  ON t.ID=s.CanteenID WHERE  s.OrderID='$submenu' AND b.User_ID='$mobile' GROUP BY d.ID");
